...
| `frame` | `KoalaFrameFolder` (numbered `{index:05d}_<stem>.<ext>` discovery + validation; mixes in `iivs.common.data`'s `FrameShapedMixin`), the `koala_frame_name` builder, `save_koala_frames` (the atomic, frame-at-a-time write of a numbered folder, shared by every modality's converter), and `detect_koala_format`. The path helper `file_extension` is imported directly from `kaparoo.filesystem`. |
