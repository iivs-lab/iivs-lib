...
class PhaseBounds:
    ...
    global min and max of that phase over the acquisition. A `PhaseFileList` can
    recompute them straight from its `Float` source via `value_range(unit=NANOMETERS)`,
    so the previews are never the authoritative source.
