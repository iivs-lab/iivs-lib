...
class IntensitySequence[T, M](DataSequence[T, M]):
    ...
    float32 (`IntensityFloatSequence`, from `Float/{Bin,Txt}`) and the uint8 display
    preview (`IntensityImageSequence`, from `Image/*.tif`). Annotate with it to accept
    any intensity sequence regardless of pixel type; annotate with the `Float` / `Image`
    subtype when the dtype matters.
...
class IntensityImageSequence[M](IntensitySequence[NDArray[np.uint8], M]):
    ...
    The display-only 8-bit preview Koala renders under `Image/*.tif`, distinct from, and
    not a substitute for, the quantitative `IntensityFloatSequence`. Same-shape sources
...
class IntensityFloatSequence[M](IntensitySequence[NDArray[np.float32], M]):
    ...
    The intensity reconstruction Koala exports as `Float/{Bin,Txt}`; annotate parameters
    with it to accept any float32 intensity source: one acquisition
    (`IntensityBinFolder`) or an arbitrary `IntensityBinList` of unrelated files, and
...
class IntensityFileList(
    ...
    An arbitrary list of float32 intensity files, each read independently. Concrete
    subclasses (`IntensityBinList`, `IntensityTxtList`) bind a format;
...
class IntensityFileFolder(
    ...
    The auto-discovered, same-shape case of `IntensityFileList`: one acquisition's
    numbered files sharing a single `header`. Concrete folders (`IntensityBinFolder`,
    `IntensityTxtFolder`) bind a format.
    """
