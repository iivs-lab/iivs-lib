...
def test_calc_drymass_respects_mask():
    ...
    whole = calc_drymass(opd, pixel_size=1e-7, alpha=0.2)
    one = calc_drymass(opd, pixel_size=1e-7, alpha=0.2, mask=mask)
...
def test_calc_drymass_from_phase_matches_two_step():
    ...
    direct = calc_drymass_from_phase(
        phase, pixel_size=1e-7, wavelength=666e-9, alpha=0.2
    )
    two_step = calc_drymass(
        phase_to_opd(phase, wavelength=666e-9), pixel_size=1e-7, alpha=0.2
    )
...
def test_calculator_calc_from_opd_matches_free_function():
    ...
    calc = DryMassCalculator(pixel_size=1e-7, alpha=0.2)
    assert calc.calc_from_opd(opd) == pytest.approx(
        calc_drymass(opd, pixel_size=1e-7, alpha=0.2)
    )
...
def test_calculator_from_wavelength_matches_free_function():
    ...
    calc = DryMassCalculator.from_wavelength(
        pixel_size=1e-7, alpha=0.2, wavelength=666e-9
    )
    assert calc.calc_from_phase(phase) == pytest.approx(
        calc_drymass_from_phase(phase, pixel_size=1e-7, alpha=0.2, wavelength=666e-9)
    )
...
def test_calculator_accepts_injected_converter():
    ...
    calc = DryMassCalculator(pixel_size=1e-7, alpha=0.2, opd_converter=conv)
...
def test_calculator_drymass_scale():
    calc = DryMassCalculator(pixel_size=1e-7, alpha=0.2)
    assert calc.drymass_scale == pytest.approx((1e-7 * 1e6) ** 2 * 1e-3 / 0.2)
