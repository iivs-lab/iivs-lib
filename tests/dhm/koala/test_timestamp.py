...
def test_txt_is_a_timestamp_sequence(tmp_path):
    assert isinstance(TXTTimestampSequence(_write(tmp_path)), TimestampSequence)
...
def test_txt_tolerates_trailing_whitespace_in_line(tmp_path):
    ...
    seq = TXTTimestampSequence(_write(tmp_path, "00000 15:21:47.674 2026.01.15 0 \n"))
    assert len(seq) == 1
...
def test_txt_ignores_blank_lines(tmp_path, text, expected_len):
    assert len(TXTTimestampSequence(_write(tmp_path, text))) == expected_len


def test_txt_rejects_malformed_line(tmp_path):
    with pytest.raises(ValueError, match="malformed"):
        TXTTimestampSequence(_write(tmp_path, "00000 15:21:47.674\n"))


def test_txt_rejects_wrong_field_format(tmp_path):
    with pytest.raises(ValueError, match="malformed"):
        TXTTimestampSequence(_write(tmp_path, "0 15:21:47.674 2026/01/15 0\n"))
...
def test_txt_rejects_noncontiguous_index(tmp_path):
    ...
    with pytest.raises(ValueError, match="index at line"):
        TXTTimestampSequence(_write(tmp_path, text))


def test_txt_missing_file_raises(tmp_path):
    with pytest.raises(FileNotFoundError):
        TXTTimestampSequence(tmp_path / "nope.txt")


def test_txt_parse_returns_timestamps(tmp_path):
    assert TXTTimestampSequence.parse(_write(tmp_path)) == (
...
def test_fps_is_a_timestamp_sequence():
    assert isinstance(
        FPSTimestampSequence(frame_rate=20.0, num_frames=3), TimestampSequence
    )


def test_fps_constant_rate():
    seq = FPSTimestampSequence(frame_rate=20.0, num_frames=3)  # 50 ms/frame
    assert len(seq) == 3
    assert seq[0] == Timestamp(elapsed_ms=0.0, interval_ms=0.0)
...
def test_fps_generate_returns_timestamps():
    assert FPSTimestampSequence.generate(frame_rate=20.0, num_frames=3) == (
        Timestamp(elapsed_ms=0.0, interval_ms=0.0),
...
def test_fps_zero_frames_is_empty():
    seq = FPSTimestampSequence(frame_rate=20.0, num_frames=0)
    assert len(seq) == 0


def test_fps_supports_negative_index_and_slice():
    seq = FPSTimestampSequence(frame_rate=20.0, num_frames=3)
    assert seq[-1] == seq[2]
...
def test_fps_rejects_nonpositive_frame_rate():
    with pytest.raises(ValueError, match="frame_rate must be positive"):
        FPSTimestampSequence(frame_rate=0.0, num_frames=3)
...
def test_fps_mean_defined_even_for_single_frame():
    ...
    seq = FPSTimestampSequence(frame_rate=20.0, num_frames=1)
    assert seq.mean_interval_ms == pytest.approx(50.0)
