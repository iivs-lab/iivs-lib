...
def test_txt_is_a_timestamp_sequence(tmp_path):
    assert isinstance(TimestampTXTSequence(_write(tmp_path)), TimestampSequence)


def test_txt_len_and_frame_indices(tmp_path):
    seq = TimestampTXTSequence(_write(tmp_path))
    assert len(seq) == 3
...
def test_txt_skips_trailing_space_and_blank_lines(tmp_path):
    seq = TimestampTXTSequence(_write(tmp_path, "00000 15:21:47.674 2026.01.15 0 \n\n"))
    assert len(seq) == 1
...
def test_txt_rejects_malformed_line(tmp_path):
    with pytest.raises(ValueError, match="malformed"):
        TimestampTXTSequence(_write(tmp_path, "00000 15:21:47.674\n"))


def test_txt_rejects_wrong_field_format(tmp_path):
    with pytest.raises(ValueError, match="malformed"):
        TimestampTXTSequence(_write(tmp_path, "0 15:21:47.674 2026/01/15 0\n"))
...
def test_txt_parse_returns_timestamps(tmp_path):
    assert TimestampTXTSequence.parse(_write(tmp_path)) == (
...
def test_fps_is_a_timestamp_sequence():
    assert isinstance(
        TimestampFPSSequence(frame_rate=20.0, num_frames=3), TimestampSequence
    )


def test_fps_constant_rate():
    seq = TimestampFPSSequence(frame_rate=20.0, num_frames=3)  # 50 ms/frame
    assert len(seq) == 3
    assert seq[0] == Timestamp(elapsed_ms=0.0, interval_ms=0.0)
...
def test_fps_generate_returns_timestamps():
    assert TimestampFPSSequence.generate(frame_rate=20.0, num_frames=3) == (
        Timestamp(elapsed_ms=0.0, interval_ms=0.0),
...
def test_fps_zero_frames_is_empty():
    seq = TimestampFPSSequence(frame_rate=20.0, num_frames=0)
    assert len(seq) == 0
...
def test_fps_mean_defined_even_for_single_frame():
    ...
    seq = TimestampFPSSequence(frame_rate=20.0, num_frames=1)
    assert seq.mean_interval_ms == pytest.approx(50.0)
...
def test_series_from_elapsed():
    assert Timestamp.series_from_elapsed([0.0, 50.0, 150.0]) == (
        Timestamp(elapsed_ms=0.0, interval_ms=0.0),
        Timestamp(elapsed_ms=50.0, interval_ms=50.0),
        Timestamp(elapsed_ms=150.0, interval_ms=100.0),
    )
    assert Timestamp.series_from_elapsed([]) == ()
